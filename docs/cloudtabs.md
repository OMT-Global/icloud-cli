...
The repo now exposes a read-only probe:
...
- Readable store: schema parsing is still not enabled until synthetic fixture coverage proves the expected tables and redaction behavior.

## Product Decision

Keep `icloud-cli safari tabs` local-session only by default. Cross-device tabs should require an explicit opt-in such as `--include-cloud` after a fixture-backed parser exists. This prevents the MVP command from unexpectedly expanding from current-Mac state to multi-device browsing state.
