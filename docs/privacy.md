...
| `icloud-cli photos list` | PhotoKit logical asset facts by default | Photos read authorization is required. The command does not request pixels, thumbnails, OCR, classification, or iCloud downloads. `photos authorization` checks state without prompting. The explicit `--degraded-filesystem` fallback may require Full Disk Access and reports degraded provenance. |
